...
namespace Chraft.Net {
    public static class PacketCryptography {
        ...
        public static string JavaHexDigest(byte[] data) {
            ...
            if (negative) hash = TwosCompliment(hash); // check for negative hashes
        ...
        public static RSAParameters GenerateKeyPair() {
            if (_provider == null) _provider = new RSACryptoServiceProvider(1024);
        ...
        private static byte[] LengthToByteArray(int octetsLength) {
            byte[] length = null;
            if (octetsLength < 0x80) { // Length: 0 <= l < 0x80
                length = new byte[1];
                length[0] = (byte)octetsLength;
            } else if (octetsLength <= 0xFF) { // 0x80 < length <= 0xFF
                length = new byte[2];
                length[0] = 0x81;
        ...
        private static byte[] CreateIntegerPos(byte[] value) {
            ...
            if (value[0] > 0x7F) {
                ...
                newInt[0] = 0x02;
                // Int Length
                for (int i = 0; i < length.Length; ++i) newInt[index++] = length[i];               
